...
KN_TEST_SUITE_BEGIN("UTF-8")
	...
	KN_TEST_UNIT("Grapheme equality") {
		...
		KN_TEST_ASSERT_TRUE(Grapheme_Is(&a, "a", 1));
		KN_TEST_ASSERT_FALSE(Grapheme_Is(&a, "b", 1));
		KN_TEST_ASSERT_FALSE(Grapheme_Is(&a, "ba", 2));
		KN_TEST_ASSERT_FALSE(Grapheme_Is(&a, "ab", 2));

		Grapheme b;
		Grapheme_Create(&b, "b", 1);
		KN_TEST_ASSERT_EQ_U8(b.byteLength, 1);
		KN_TEST_ASSERT_EQ_U8(b.codePointLength, 1);
		KN_TEST_ASSERT_TRUE(Grapheme_Is(&b, "b", 1));
		...
		KN_TEST_ASSERT_TRUE(Grapheme_Is(&extended, "\xe2\x80\xa2\xe2\x88\x99", 2));
		KN_TEST_ASSERT_FALSE(Grapheme_Is(&extended, "\xe2\x80\xa2\xe2\x88\x98", 2));
	}

	KN_TEST_UNIT("Grapheme equality") {
		Grapheme a;
		Grapheme_Create(&a, "a", 1);
		KN_TEST_ASSERT_TRUE(Grapheme_Equal(&a, &a));

		Grapheme abc;
		Grapheme_Create(&abc, "abc", 3);
		KN_TEST_ASSERT_TRUE(Grapheme_Equal(&abc, &abc));

		KN_TEST_ASSERT_FALSE(Grapheme_Equal(&a, &abc));

		KN_TEST_PRECONDITION(Grapheme_Equal(NULL, &a));
		KN_TEST_PRECONDITION(Grapheme_Equal(&a, NULL));
	}
	...
	KN_TEST_UNIT("Building code point sequences") {
		...
		Grapheme abc;
		Grapheme_Create(&abc, "abc", 3);
		KN_TEST_ASSERT_TRUE(Grapheme_Equal(&a, &abc));
	}
