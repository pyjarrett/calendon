...
float Angle_ToDegrees(PlanarAngle a)
{
	return a.degrees;
}

float Angle_ToRadians(PlanarAngle a)
{
	return a.degrees * pi / 180.0f;
}

PlanarAngle Angle_InDegrees(float d)
{
	return (PlanarAngle) { d };
}

PlanarAngle Angle_InRadians(float r)
{
	return (PlanarAngle) { r * 180.0f / pi };
}

PlanarAngle Angle_Add(PlanarAngle left, PlanarAngle right)
{
	return (PlanarAngle) { left.degrees + right.degrees };
}

PlanarAngle Angle_Sub(PlanarAngle left, PlanarAngle right)
{
	return (PlanarAngle) { left.degrees - right.degrees };
}

float2 float2_Make(float x, float y)
{
	return (float2) { x, y };
}

float2 float2_FromPolar(float radius, PlanarAngle theta)
{
	return (float2) { radius * cosf(Angle_ToRadians(theta)),
		radius * sinf(Angle_ToRadians(theta)) };
}

float2 float2_Add(float2 left, float2 right)
{
	return (float2) { left.x + right.x, left.y + right.y };
}

float2 float2_Sub(float2 left, float2 right)
{
	return (float2) { left.x - right.x, left.y - right.y };
}

float2 float2_Multiply(float2 v, float s)
{
...
float float2_LengthSquared(float2 v)
{
...
float2 float2_Midpoint(float2 left, float2 right)
{
	return float2_Divide(float2_Add(left, right), 2.0f);
}

float float2_DistanceSquared(float2 left, float2 right)
{
	return float2_LengthSquared(float2_Make(left.x - right.x, left.y - right.y));
}

float2 float2_Lerp(float2 from, float2 to, float alpha)
{
...
PlanarAngle float2_DirectionBetween(float2 from, float2 to)
{
