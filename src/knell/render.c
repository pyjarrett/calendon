...
bool R_CreateSprite(SpriteId* id)
{
	KN_ASSERT(id != NULL, "Cannot assign a sprite to a null pointer.");
	return RLL_CreateSprite(id);
}

bool R_LoadSprite(SpriteId id, const char* path)
{
	return RLL_LoadSprite(id, path);
}

void R_DrawSprite(SpriteId id, float2 position, Dimension2f size)
{
...
void R_DrawDebugFullScreenRect(void)
{
	RLL_DrawDebugFullScreenRect();
}

void R_DrawDebugRect(float2 center, Dimension2f dimensions, rgb8 color)
{
	RLL_DrawDebugRect(center, dimensions, float4_Make((float)color.r / 255.0f,
		(float)color.g / 255.0f, (float)color.b / 255.0f, 1.0f));
}

void R_DrawDebugLine(float x1, float y1, float x2, float y2, rgb8 color)
{
	RLL_DrawDebugLine(x1, y1, x2, y2, color);
}

void R_DrawDebugLineStrip(float2* points, uint32_t numPoints, rgb8 color)
{
	RLL_DrawDebugLineStrip(points, numPoints, color);
}

void R_DrawDebugFont(FontId id, float2 center, Dimension2f size)
{
	RLL_DrawDebugFont(id, center, size);
}

void R_OutlineCircle(float2 center, float radius, rgb8 color, uint32_t numSegments)
{
